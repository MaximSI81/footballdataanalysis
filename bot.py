...
SUB_CATEGORIES = {
    ...
    "players": {
        "📊 Анализ формы всех игроков": "players_all",
        "🔙 Назад к разделам": "back_to_main"
    }
}
...
SECTION_MAPPING = {
    ...
    "attack_zones": {
        "sections": ["🔄 АКТИВНОСТЬ ФЛАНГОВ", "🎯 ДАЛЬНИЕ АТАКИ", 
                    "🎯 АНАЛИЗ ЗОН АТАК И УЯЗВИМОСТЕЙ", "🏹 АТАКУЕТ"]
    },
    "attack_all": {
    ...
    "stats_all": {
        "sections": ["📊 КОНТРОЛЬ ИГРЫ", "🔄 ТОЧНОСТЬ ПЕРЕДАЧ", "🛡️ ОБОРОНА", "🎪 КАЧЕСТВО МОМЕНТОВ"]
    },
    ...
    "players_all": {
        "sections": [
            "🏷️", "Общий рейтинг:", "⚽ НАПАДАЮЩИЕ", "🎯 ПОЛУЗАЩИТНИКИ", 
            "🛡️ ЗАЩИТНИКИ", "🥅 ВРАТАРИ"
        ]
    }
}
...
async def select_sub_category(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Обработка анализа игроков
    if data_type == "players_all":
...
def filter_output(output: str, data_type: str) -> str:
    ...
    for line in lines:
        ...
        section_keywords = [
            ...
            "Общий рейтинг:", "НАПАДАЮЩИЕ", "ПОЛУЗАЩИТНИКИ", "ЗАЩИТНИКИ", "ВРАТАРИ"
        ]
...
def get_brief_overview(output: str, data_type: str) -> str:
    ...
    brief_messages = {
        ...
        # Игроки
        "players_all": "📊 Анализ формы игроков временно недоступен"
    }
